// khandaia-relay — Cloudflare Worker v1
// Relay proxy cho Khán Đài A API — bypass 403 từ GitHub Actions IP
// POST / { api_base? }  →  GET {api_base}/fixtures/unfinished
// Auth: X-Relay-Token header == RELAY_SECRET env binding

const KHANDAIA_FRONTS = [
  "https://tructiep.khandaia.link",
  "https://khandaia.link",
];
...
async function discoverApiBase() {
  for (const front of KHANDAIA_FRONTS) {
    try {
      ...
      for (const s of scripts.slice(0, 8)) {
        try {
          ...
          const hit = js.match(/baseURL\s*:\s*["'](https:\/\/sv[^"']+)["']/)
            || js.match(/["'](https:\/\/sv\.khandai[^"']+)["']/)
            || js.match(/["'](https:\/\/api\.khandai[^"']+)["']/);
          if (hit) return hit[1].replace(/\/$/, "");
        } catch (_) {}
      }
    } catch (_) {}
  }
  return "https://sv.khandai-a.xyz/api/v1/external";
}
...
async function getBase(env) {
  const envApi = (env && env.KHANDAIA_API) || "";
...
export default {
  async fetch(req, env) {
    ...
    if (path === "/healthz") {
      return jsonResp({
        ok: true,
        worker: "khandaia-relay",
        env_api: (env && env.KHANDAIA_API) || "(not set)",
    ...
    const frontendUrl = KHANDAIA_FRONTS[0];
